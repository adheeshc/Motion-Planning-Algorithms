...
# Last Modified time: 2020-03-22 01:31:29
...
class RRT_star():
	def __init__(self, grid_size,start_x, start_y,goal_x,goal_y,randArea, res=1,robot_size=1, expand_dist=1.0, sample_rate=5, max_iter=2000):
	...
	def algorithm(self):
		...
		for i in range(self.max_iter):
			...
			#HIT OBSTACLES
			if self.verify_node(current,self.ob_map,0,0,self.map_x,self.map_y):
				nearinds = self.find_near_nodes(current)
				current = self.choose_ind(current, nearinds)

				#ADD NODE TO LIST
				self.node_list.append(current)

				#REWIRE TREE
				self.rewire(current, nearinds)
	...
	def find_near_nodes(self,current):
		...
		r = 50*np.sqrt((math.log(n_node)/n_node))
